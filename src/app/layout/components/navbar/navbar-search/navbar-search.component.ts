...
export class NavbarSearchComponent implements OnInit {
  ...
  @HostListener('keydown.escape') fn() {
    ...
    this.searchText = '';
    this._collaborators.changeMessage('');
  }
   ...
   */
  toggleSearch() {
   // [routerLink]="['/main/profile/view'] console.log('habre lupa');
  //  this._searchService.onIsBookmarkOpenChange.next(false);
    ...
    if (this.openSearchRef === false) {
     ...
     this.router.navigate(['/main/profile/list']); 
  }
  ...
  searchUpdate(event) {
   /* const val = event.target.value.toLowerCase();
    ...
    this._collaborators.changeMessage(this.searchText);
